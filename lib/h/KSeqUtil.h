...
class KSeqUtil
{
	public:
		...
		bool ByteSwap(unsigned char* pucBuf, short size);										// 2020.02.21 - htons와 같은 동작 little Endian <-> Big Endian시 사용
};
...
class kUtlFile
{
	public:
		...
		int KaGetFileCount( char* pszFolder, char* pszFileName, bool bSearchSub );					// 2019.12.25 - 지정된 폴더의 파일의 갯수를 구한다.
};
